module ConfigDrivenHelper
      ...
      def merge_default_shared_site(node, name, site, server_type = nil)
        ...
        site = self::immutablemash_to_hash(site) if site.is_a?(Chef::Node::ImmutableMash)

        if site['inherits']
          site = ::Chef::Mixin::DeepMerge.hash_only_merge(
            self::immutablemash_to_hash(node[type]['shared_config'][site['inherits']]),
            site)
        end

        site = ::Chef::Mixin::DeepMerge.hash_only_merge(self::immutablemash_to_hash(node["#{type}-sites"]), site)
...
Chef::Recipe.send(:include, ConfigDrivenHelper)
